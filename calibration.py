...
class Calib(object):
        ...
        if not flag:
            ...
            if s_err > ORT:
                ...
                if sort_mod == 'gt':
                    ...
                    print('corner error=', err)
                    corner_flag = flag
                # print(f'{name} image subpixel corners got with sigma error {s_err}')
                # cs = self.subpixel_exactor.sort_by_gt_data(False); #optional
                cs = np.array(cs)
        else:
    ...
    def calib_by_iter(self, heatmap_folder, subpixel_path,  err_thred, max_iter, ref_K=None, max_pose_num=20, sort_mod='gt', ORT=20, ref_path=r'',img_path='', ref_mod='gt', draw_flag=False, save_flag=False, draw_threshold=20):
        """Use reference to calibration iteratively
        """
        # save subpixel coordinates
        if save_flag:
            print('Get sub-pixel corners...')
            self.save_subpixel_corners(heatmap_folder, sort_mod,ORT,ref_path,img_path,save_path=subpixel_path)
        ...
        while err_ip > err_thred and iter_count < max_iter:
            name_list = self.__choose_pose_randomly(subpixel_path, max_pose_num)
            pose_num = len(name_list)
            ret, mtx, dist, _, _ = self.calib_by_subpixel_res(name_list)
            if ref_mod == 'gt':
                err_f, err_p, err_ip = self.cal_accuracy_by_gt(ref_path, mtx)
                # if ret < draw_threshold and err_ip < 50:
                # if ret <= 0.5 and ret >= 0.45 and err_ip < 15:
                errs_f.append(err_f)
                errs_p.append(err_p)
                errs_ip.append(err_ip)
                rets.append(ret)
                pose_nums.append(pose_num)

            elif ref_mod == 'given':
                err_f, err_p, err_ip = self.cal_accuracy_by_ref_K(ref_K,mtx)
                if ret < draw_threshold:
                    errs_f.append(err_f)
                    errs_p.append(err_p)
                    errs_ip.append(err_ip)
                    rets.append(ret)
            ...
            if err_ip < min_err:
                ...
                min_name = name_list
            iter_count += 1
            print(f'{iter_count} Minimal error currently = {min_err}')
        
        if draw_flag:
            # self.draw_error_distribution(errs_f, errs_p)
            # self.draw_ret_and_err_ip_distribution(errs_ip, rets)
            self.draw_ret_and_err_ip_distribution(errs_ip, pose_nums)

        if iter_count==max_iter:
            logging.info(f'No Good Pose! Calibration failed with minimal error = {min_err}.\nThe min error name is {min_name}')
        else:
            ...
            for name in name_list:
                print(name)
            if ref_mod == 'gt':
                self.show_accuracy_by_gt(ref_path, mtx)
            elif ref_mod == 'given':
                self.show_accuracy_by_ref_K(ref_K, mtx)

    def calib_by_RANSAC(self, heatmap_folder, subpixel_path, max_iter_num=100, least_pose_num=20, outlier_threshold=0.8, inlier_threshold=2/3, sort_mod='gt', ORT=20, ref_path=r'', img_path=r'', ref_mod='gt',save_flag=False, draw_flag=False):
        """
        Args:
        ...
        while inlier_num < inlier_num_threshold and iter_count < max_iter_num:
            ...
            # for test
            _,_,e_ip = self.cal_accuracy_by_gt(ref_path, mtx)
            IPs.append(e_ip)
            I_nums.append(count)
            ...
            if inlier_num > inlier_max:
                ...
                if mean_RPE < min_RPE:
                    best_models.clear()
                    best_models.append(mtx)
                elif mean_RPE > min_RPE:
                    pass
                else:
                    best_models.append(mtx)
                if inlier_max_ip_error > e_ip:
                    inlier_max_ip_error = e_ip
            
            iter_count+=1
            print(f'{iter_count} iter with max {inlier_max} inliers and IP error={inlier_max_ip_error} RPE = {min_RPE}')

        
        if draw_flag:
            self.draw_error_distribution(IPs, I_nums)
    ...
    def calib_by_RANSAC_practical(self, heatmap_folder, subpixel_path, max_iter_num=100, least_pose_num=20, outlier_threshold=0.8, inlier_threshold=2/3, sort_mod='gt', ORT=20, ref_path=r'', ref_mod='gt',save_flag=False, draw_flag=False):
        ...
        while inlier_num < inlier_num_threshold and iter_count < max_iter_num:
            ...
            if inlier_num > inlier_max:
                ...
                best_models.append(mtx)
            iter_count+=1
            # print(f'{iter_count} iter with max {inlier_max} inliers and IP error={inlier_max_ip_error}')
